// This file was changed from the original nvpatch source. See NOTICE in this directory.
// The change is one line in WriteExportTable, marked with a MODIFIED comment.
...
namespace nvpatch
{
    ...
    unsafe class PEExportTable
    {
        ...
        public DataDirectory Write(PESectionBuilder sect)
        {
            ...
            // MODIFIED from upstream: was OrderBy(x => x.Name) (default comparator)
            // The PE format requires the name pointer table in ordinal order,
            // because GetProcAddress binary searches it.
            var sorted = _all.OrderBy(x => x.Name, StringComparer.Ordinal).ToList();
