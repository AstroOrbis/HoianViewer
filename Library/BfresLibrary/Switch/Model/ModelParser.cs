    ...
    public class ModelParser
    {
        ...
        public static void Write(ResFileSwitchSaver saver, Model model)
        {
            ...
            if (saver.ResFile.VersionMajor >= 10)
            {
                ...
                model.ShaderAssign = new List<MaterialParserV10.ShaderAssignV10>();
                foreach (var mat in model.Materials.Values)
                {
                    var own = mat.ShaderInfoV10.ShaderAssign;
                    var shared = model.ShaderAssign.FirstOrDefault(x => x.Matches(own));
                    if (shared == null)
                        model.ShaderAssign.Add(own);
                    else
                        mat.ShaderInfoV10.ShaderAssign = shared;
                }
            }
